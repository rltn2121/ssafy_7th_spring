...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <div class="container-fluid">
    <a class="navbar-brand" href="#"><img src="<%= staticPath + userDto.getUserProfileImageUrl() %>" alt="Logo" style="width:24px; height: 24px; border-radius: 50%;"></a>
...
async function boardList(){
	...
	let url = '<%=contextPath%>/boards'; // eclipse 와 다른 부분
	...
	try{
        ...
        if( data.result == "login") {
        	window.location.href="<%=contextPath%>/login"; // UserServlet doGet()
...
async function boardDetail(boardId){
	...
	let url = '<%=contextPath%>/boards/' + boardId; // eclipse 와 다른 부분
		...
		fileList.forEach( el => {
			...
			fileListDetailHtml += 
				...
				<div>
					<span class="fileName">\${fileName}</span>
					&nbsp;&nbsp;
					<a type="button" class="btn btn-outline btn-default btn-xs" data-fileId="\${fileId}" href="<%=staticPath%>/\${fileUrl}" download="\${fileName}">내려받기</a>	
				</div>
				`;			
		});
